...
class MSDWebView: UIView {
    ...
    /// WebView配置项
    var webConfig : MSDWebViewConfig?
    
    //保存请求链接
    fileprivate var snapShotsArray:Array<Any>?
    
    //设置代理
    weak var delegate : MSDWebViewDelegate?
    ...
    fileprivate func setupUI(webConfig:MSDWebViewConfig)  {
        // Webview的偏好设置
    ...
    /// 加载webView
    func webloadType(_ target:AnyObject,_ loadType:MSDWebLoadType) {
        self.target = target
        setupUI(webConfig:webConfig ?? MSDWebViewConfig())
